...
int main()
{
	...
	while(1)
	{
		...
		if(funcao == 1)
		{
			...
			//verificar se o E(phi) é relativamente primo ao produto (p-1)*(q-1).
			verificar(e, p, q, &e);
			printf("\nGerando chave pública...\nChave pública gerada!\n");

			//após verificação um arquivo é criado com a chave pública gerada.
			criararquivo(p*q,e);
			printf("\n\nCaso deseje voltar ao menu, digite 1 e caso não, digite 0 para encerrar o programa: ");
		...
		// Executa os comandos atribuidos a função 2 - Criptografar
		if (funcao == 2)
		{
			printf("--- Função 2 ---\n\n");
			...
			//Inicialmente será solicitado o texto a ser criptografado
			fgets(frase, 4000, stdin);

			//Logo após, será solicitado a chave pública, para criptografar o texto.
			//O texto será criptografado com base nos valores de (P*Q) e E(phi).
			printf("\nDigite a chave pública para criptografar:");
			scanf("%lld%lld", &n, &e);
			printf("Aguarde, estamos criptografando sua mensagem...\n");

			//Nessa etapa os valores de cada letra basearam-se na Tabela ASCII.
			//Considerando apenas as letras maiusculas, e enquadrando o alfabeto no intervalo de 2 à 28, incluindo o espaço.
			...
			printf("Pronto! Sua mensagem foi criptografada.\n");

			//Após é criado um txt contendo a frase criptografada
		...
		// Executa os comandos atribuidos a função 3 - Descriptografar
		if(funcao == 3)
		{
			printf("--- Função 3 ---\n\n");
			printf("Digite os números primos p e q:");
			...
			//Inserir o mesmo valor de E(phi), utilizado na chave pública.
			scanf("%lld", &e);

			//verificar se o E(phi) é relativamente primo ao produto (p-1)*(q-1).
			verificar(e, p, q, &e);

			//A leitura do arquivo txt da mensagem criptografada é feita
			FILE *chav;
			chav= fopen("cripitografado.txt", "r");
			contador = 0;
			printf("Aguarde, estamos descriptografando a sua mensagem agora...\n");
			while (!feof(chav))
			{
			...
			//Após a leitura da mensagem e dos valores de P e Q e E(phi)
			//A descriptografia é feita
			descriptografar(p, q, e, frasen, frase, (contador - 1));

			//Um arquivo txt é criado com frase descriptografada
			criararquivodecr(frase);
			printf("Pronto! Sua mensagem foi descriptografada.\n");
			printf("\n\nCaso deseje voltar ao menu, digite 1 e caso não, digite 0 para encerrar o programa: ");
			scanf("%d", &sair);
